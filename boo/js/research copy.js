    ...
    function checkMatch(text, search, skill = null) {
        if (typeof search === 'string') {
            ...
            if (match) {
                ...
                // 一部スキルでは単語の頭を大文字にする
                if (matchText) {
                    if (['Alchemy', 'Anatomy', 'Fishing', 'Crafting', 'Enchanting', 'SkillLevelUp'].includes(skill)) {
        ...
        if (lineMatch) {
            ...
            if (matchedText) {
                ...
                if (skillCode === 'Alchemy') {
                    ...
                    potionCount[potionName] ||= {};
                    potionCount[potionName][skillAction] = (potionCount[potionName][skillAction] || 0) + 1;
                    totalPotionCount[potionName] ||= {};
                    totalPotionCount[potionName][skillAction] = (totalPotionCount[potionName][skillAction] || 0) + 1;
                ...
                if (skillCode === 'Crafting') {
                    ...
                    if (lastCraftSkill) {
                        const craftStuff = matchedText;
                        skillCount[lastCraftSkill] ||= {};
                        skillCount[lastCraftSkill][skillAction] = (skillCount[lastCraftSkill][skillAction] || 0) + 1;
                        totalSkillCount[lastCraftSkill] ||= {};
                        totalSkillCount[lastCraftSkill][skillAction] = (totalSkillCount[lastCraftSkill][skillAction] || 0) + 1;
                        if (lastCraftSkill === 'Blacksmithing') {
                            blacksmithCount[craftStuff] ||= {};
                            blacksmithCount[craftStuff][skillAction] = (blacksmithCount[craftStuff][skillAction] || 0) + 1;
                            totalBlacksmithCount[craftStuff] ||= {};
                            totalBlacksmithCount[craftStuff][skillAction] = (totalBlacksmithCount[craftStuff][skillAction] || 0) + 1;
                        }
                        if (lastCraftSkill === 'Tailoring') {
                            tailorCount[craftStuff] ||= {};
                            tailorCount[craftStuff][skillAction] = (tailorCount[craftStuff][skillAction] || 0) + 1;
                            totalTailorCount[craftStuff] ||= {};
                            totalTailorCount[craftStuff][skillAction] = (totalTailorCount[craftStuff][skillAction] || 0) + 1;
                        }
                        if (lastCraftSkill === 'Tinkering') {
                            tinkerCount[craftStuff] ||= {};
                            tinkerCount[craftStuff][skillAction] = (tinkerCount[craftStuff][skillAction] || 0) + 1;
                            totalTinkerCount[craftStuff] ||= {};
                            totalTinkerCount[craftStuff][skillAction] = (totalTinkerCount[craftStuff][skillAction] || 0) + 1;
                        }
                        if (lastCraftSkill === 'Woodcrafting') {
                            woodcraftCount[craftStuff] ||= {};
                            woodcraftCount[craftStuff][skillAction] = (woodcraftCount[craftStuff][skillAction] || 0) + 1;
                            totalWoodcraftCount[craftStuff] ||= {};
                            totalWoodcraftCount[craftStuff][skillAction] = (totalWoodcraftCount[craftStuff][skillAction] || 0) + 1;
                        }
            ...
            if (SKILLS.includes(skillName)) {
                ...
                if (skillCode === 'Alchemy') {
                    if (Object.keys(potionCount).length && optionPotion) {
                        resultLogs.push(`${SPACER}============================`);
                        const sortedPotionCount = Object.entries(potionCount).sort((a, b) => b[1].start - a[1].start);
                        const maxLength = Math.max(...Object.values(potionCount).map(v => String(v.success).length));
                        let reagent = {};
                        for (const [potionName, actions] of sortedPotionCount) {
                            const potion = POTION[potionName];
                            if (optionPotion) {
                                resultLogs.push(`${SPACER}${String(actions.success).padStart(maxLength, ' ')} ${potionName}`);
                            }
                            // 消費した試薬の計算
                            for (const [key, value] of Object.entries(potion.reagent)) {
                                reagent[key] = (reagent[key] || 0) + value * actions.success;
                            }
                        }
                        potionCount = {};
                        // 消費した試薬の表示
                ...
                // Blacksmithing
                if (skillCode === 'Blacksmithing') {
                }
                // Tailoring
                if (skillCode === 'Tailoring') {
                    
                }
                // Tinkering
                if (skillCode === 'Tinkering') {
                    
                }
                // Woodcrafting
                if (skillCode === 'Woodcrafting') {
